...
// Resolves the image URL for a given base symbol (e.g. "BTC") from CoinGecko.
// Returns an empty string if not found — the card component handles the letter fallback.
export async function getImageUrl(baseSymbol) {
  const images = await getCoinImages();
  return images[baseSymbol.toLowerCase()] || '';
}
